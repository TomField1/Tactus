        ...
        for (int i = 0; i <= Maze.GetUpperBound(0); i++)
        {
            for (int j = 0; j <= Maze.GetUpperBound(1); j++)
            {
                if (Maze[i, j] == 1)
                {
                    ...
                    ptype.transform.position = new Vector3(i * ptype.transform.localScale.x - width/2 * ptype.transform.localScale.x, 0, j * ptype.transform.localScale.z - height/2 * ptype.transform.localScale.z);
                    if (brick != null)
                    {
